...
import com.eshope.admin.CSVExporter.CategoryCsvExporter;
import com.eshope.admin.ExcelExporter.CategoryExcelExporter;
import com.eshope.admin.PDFExporter.CategoryPdfExporter;
    ...
    public String editCategorySave(RedirectAttributes redirectAttributes, @Valid @ModelAttribute(value = "category") Category category, Errors errors, Model model, @RequestParam("image")MultipartFile multipartFile) throws IOException{
        ...
        if (existingCategory!=null && !(existingCategory.getName().matches(category.getName()))) {
            if (category.getName() != "" && !categoryService.isNameUnique(category.getName())) {
                ...
                List<Category> listCategories = categoryService.listCategoriesUsedInForm();
                model.addAttribute("listCategories", listCategories);
                model.addAttribute("category1",existingCategory);
                return "Category/categoryUpdateForm.html";
            }
        }
